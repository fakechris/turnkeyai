...
| # | 项目 | 优先级 | 预估规模 | 依赖 |
|---|------|--------|---------|------|
| 1 | 浏览器副作用执行点审批 gate | P0（安全） | L（~1-2 周） | 无 |
| 2 | 记忆生命周期与持久化安全 | P1（数据安全） | M（~3-5 天） | 无 |
| 3 | memory writer 增量读取 | P1（性能，随线程长度恶化） | M（~3-5 天） | 无 |
| 4 | crash 后工作项 reconcile + journal I/O | P1/P2（稳定性） | M | 无 |
| 5 | prompt registry 治理闭环 | P2（治理/可观测） | S-M（~2-3 天） | 无 |

各项目相互独立，可并行；建议先启动 1 和 2。
