...
| 4b | run-journal 效应转换 WAL | P2（效率） | M | 无 | ✅ 已交付（PR #548，watermark-gated WAL） |
| 5 | prompt registry 治理闭环 | P2（治理/可观测） | S-M（~2-3 天） | 无 | ✅ 已交付（PR #546） |

**交付进度（2026-07-24）**：6 个项目（2、3、4a、4b、5，及项目 2 的持久化/生命
周期子项）已实现、测试、合并入 main（PR #544/#545/#546/#548，均全量测试绿 +
CI 绿）。**唯一剩余的是项目 1**（P0 浏览器执行点 gate，跨 worker-runtime/
browser-bridge 协议，需版本兼容）——全 plan 中风险最高、影响面最大的一项，
按决策留作专注、可充分评审的专项。下文其计划保持不变，供承接时使用。

- 项目 2 的**衰减子项（2.4 时效衰减）**已刻意延后，避免扰动已调优的 recall
  排序；其余持久化/过期/淘汰/近重复合并已交付。
- 项目 4b 的崩溃恢复正确性由确定性的 crash-injection 单测 + chaos e2e 验证；
  真实 LLM 的审批/重启续跑场景本就在 main 上同样 flaky（已核实非本项引入）。
- 项目 1 是唯一剩余项，需专项承接。
