...
| Metric | Value |
|---|---|
| Inline behavior tests (baseline, `npm run parity:inline`) | **267 pass / 0 fail** (grew from 252 as each batch added grouped parity + golden-order tests) |
| Engine tests run to completion | **266** (1 skipped — see below) |
| Pass on engine | **185** (A) → **222** (B) → **260** (C/D/E + P2 fixes) |
| Fail on engine | **65** (A) → **28** (B) → **6** (C/D/E). B: T2 40→7, T10 11→8, Other 5→4. C: T10 8→0. D: C5 5→0. E: T7 4→0. |
| Incomplete after recovery | **0** (chunked + per-test re-run recovers cross-test leak crashes) |
| Skipped — known engine non-termination | **1**: (Batch B) `does not treat resumable partial session output as completion evidence` — engine churns to `maxRounds` even in isolation where inline converges (a continuation-completion divergence, next-campaign work). **#55 is FIXED** (Batch E): the parent wall-clock boundary test no longer crashes — the engine disposes the per-chunk wall-clock signal and extends (never aborts) an active browser session, so no timer leaks; it was removed from `KNOWN_HANGS`. |

**Harness note.** Before the Batch E fix a single-process engine run died after ~54 tests because a leaked browser-session timer crashed whatever test was executing when it fired. The runner still executes the suite in small fresh-process chunks (so any future leak at most kills one chunk), force-exits each chunk, applies a per-test timeout, reaps the process group on an OS backstop, and re-runs any unreported test individually to recover blameless neighbours and isolate a crasher. This is what makes parity "known, categorized, and runnable to completion."

### Remaining 6 engine fails (next-campaign scope)

Full per-test list lives in `docs/STAGE8B_PARITY_STATUS.md` (regenerated by `npm run parity:engine`). After Batches A–E + the codex-boundary P2 fixes, the residual surface is:

| Cluster | Fails | Notes |
|---|---|---|
| **T2 continuation/probe** | **4** | continuation-lookup + browser-probe-after-timeout; need the inline-only `enforceSupplementalLocalTimeoutProbeToolCall` normalizer + `findSessionContinuationLookupDirective` (a T5 gap) |
| **Other** | **2** | `runs native tool-use loop and feeds tool results back`; `disables native tools when no-tool-use packet` |

These 6 + the 1 Batch B convergence skip are the proposed **Stage 8 final parity campaign** (a separate PR after this C/D/E campaign merges).

**Now DONE on the engine path.** Merged #515–#526 (approval/permission/progress/persistence) plus Batches A–E: tool normalization + continuation (B), browser/session finalization (C), memory/compaction/envelope (D), execution budget + wall-clock + #55 (E). Codex review of C/D/E found 3 [P2] engine-only divergences (round-limit tool-free final, natural-finish epilogue context, over-cap started events); all fixed, tests #1/#3 mutation-verified.
