...
> `onTerminate`, the finalization appenders, and the observability bridge).
> Old historical backlog wording should not appear in this table anymore.
> If it does, treat that as a documentation bug, not a parity backlog. The only
> columns that still point at *future* work are **`regex?`** and
> **`typedFactsNeeded`**: those are Stage 8C **architecture / typed-facts debt** (§4),
> not parity gaps. Do not treat a `regex? = yes` row as something to "port" — it
> already runs; it is a candidate to re-express with typed facts later.
>
...
| behaviorName | inlineLocation | trigger | action | precedence | engineStatus | regex? | typedFactsNeeded | relatedTests | notes |
|---|---|---|---|---|---|---|---|---|---|
| enforceSupplementalLocalTimeoutProbeToolCall | :473-484 (engine pipeline :228-249) | latest msg includes 'Runtime correction: resumed timeout evidence is still content-poor' AND toolCalls>0 | rewrite all pending calls → single sessions_spawn (supplemental probe task + URL + bounded timeout_seconds) | 2nd in pipeline (after enforceMissingApprovalGate, before applySessionContinuationDirective :489) | in_engine | yes | structured 'resumed timeout evidence' status field (now text-detected) | test:24316 | Engine normalization pipeline includes this step; remaining work is a typed probe-pending fact. |
...
| normalizeSessionToolAliasCalls | :475 | toolCall.name in SESSION_SEND_ALIAS_NAMES | map alias → sessions_send + extract session_key/message from variously-named fields | innermost initial wrapping :475 | in_engine | no | none (schema-only) | — | Runs in the engine normalization pipeline. |
| applySessionContinuationDirective | :489 and :507 (called twice) | sessionContinuationDirective!=null AND toolCalls>0 | merge directive.sessionKey into sessions_send (filter spawn/history/list) OR rewrite first spawn/history/list → sessions_send | 1st explicit step :489; AGAIN after normalizeApprovalGatedBrowserSpawnCalls :507 | in_engine | no | SessionContinuationDirective (already structured) | — | Runs twice in the engine pipeline to preserve inline precedence. |
| applySessionContinuationLookupDirective | :490 | sessionContinuationLookupDirective!=null AND toolCalls>0 | route to sessions_list (from send/spawn) OR filter spawn/send if list exists | 2nd step :490 | in_engine | no | SessionContinuationLookupDirective (already structured) | — | Runs in the engine normalization pipeline. |
| normalizeExplicitContinuationHistoryCalls | :491 | taskPrompt explicit continuation (continue/resume/retry + existing/same/previous OR requestsTimeoutFollowupContinuation) AND not transcript request | sessions_history → sessions_send w/ merged or default 'Continue this existing sub-agent session' | 3rd step :491 | in_engine | yes | continuation-intent fact from TaskFacts (now regex on taskPrompt) | — | Runs in the engine pipeline; remaining work is typed continuation intent. |
| normalizeSessionToolCalls | :492 | sessions_send/sessions_history AND session_key matches known worker pattern AND resolvable | normalize session_key via extractWorkerSessionKey + resolveKnownWorkerSessionKey | 4th step :492 | in_engine | no | known-session-keys from SessionContinuationState (now string parse) | — | Runs in the engine normalization pipeline. |
| normalizePrivateUrlResearchSpawnCalls | :493-497 | browserAvailable AND sessions_spawn AND agent_id='explore' AND private/loopback URL OR toolCallTargetsBrowserRequiredUrl | rewrite agent_id explore→browser OR skip if loopback read-only (allowsLoopbackExploreForE2E) | 5th step :493-497 | in_engine | yes | URL classification (private/loopback/public); browser-required-URL fact (now containsPrivateOrLoopbackHttpUrl + toolCallTargetsBrowserRequiredUrl) | — | Runs in the engine pipeline; remaining work is typed URL/browser-required facts. |
| normalizeLocalUrlWebFetchCalls | :498 | web_fetch AND private/loopback HTTP URL | web_fetch → sessions_spawn agent_id='browser' label='local-url-fetch' | 6th step :498 | in_engine | yes | URL classification (now containsPrivateOrLoopbackHttpUrl + extractHttpUrls) | — | Runs in the engine pipeline; remaining work is typed URL facts. |
| normalizeBoundedTimeoutSourceSpawnAgents | :499-503 | exploreAvailable AND looksBoundedTimeoutSourceCheck AND sessions_spawn agent_id='browser' AND HTTP URL AND browserRequired fails | rewrite browser→explore OR skip if browser-required | 7th step :499-503 | in_engine | yes | browser-required-URL fact (taskRequiresBrowserEvidence + toolCallTargetsBrowserRequiredUrl); timeout-source classification | — | Runs in the engine pipeline; remaining work is typed timeout-source facts. |
| normalizeBoundedTimeoutDuplicateSourceSpawns | :504-506 | looksBoundedTimeoutSourceCheck AND multiple sessions_spawn same normalized URL | keep 1 spawn/URL (highest score), drop duplicates | 8th step :504-506 | in_engine | yes | URL dedup (extractHttpUrls + normalizeUrlForComparison); browser-required scoring | — | Runs in the engine pipeline; remaining work is typed URL dedup facts. |
| normalizeApprovalGatedBrowserSpawnCalls | :508-512 (engine onToolCalls :2542-2557) | sessions_spawn agent_id='browser' AND looksApprovalGatedBrowserSideEffect AND mutating AND not already-applied AND no permission evidence AND no permission_* pending | insert buildPreApprovalBrowserInspectionSpawn + buildPermissionQueryFromBrowserSpawn OR filter duplicate browser spawns | 9th step :508-512; engine pipeline preserves the safety invariant | in_engine | yes | approval-gated-action + mutating-action classification; typed permission evidence from EvidenceLedger | test:24235 (comment, engine mirrors at :513) | Runs in the engine pipeline; remaining work is typed mutation/permission facts. |
...
| canonicalizeSessionToolTraceCalls | ~5801 (inline :1540) | post-result recording | compare trace calls vs parsed result.session_key; rewrite call.input to match | after execution, before compact/finalization | in_engine | no | parseSessionToolResult typed; direct field compare | session_key normalized; unmodified when match | Engine behavior is covered by parity; future cleanup should keep trace canonicalization in a typed evidence layer. |
...
| findSessionContinuationLookupDirective | ~9512 (engine :620-625,:2741,:2810) | activeToolLoop AND !directive AND !probePending AND continuation-looking taskPrompt; guarded by contextHasSessionListResult | inject sessions_list lookup; return {messageHint} or null | fallback to directive :468; feeds applySessionContinuationLookupDirective :490 | in_engine | yes | continuation-intent field; structured session list result; typed lookup directive producer | lookup when list exists; lookup suppression when direct avail | Lookup injection is ported in the engine empty-round and normalization paths; remaining work is typed continuation intent. |
...
| tool_evidence_fallback (closeout) | :420-431 (engine onModelCallError :4915-4965 and onRepairRound closeout :3971-3980) | activeToolLoop AND generate throws RequestEnvelopeOverflowError/buildLocalEvidenceCloseoutResult succeeds, OR hard approval wait-timeout fallback fires | metadata reason='tool_evidence_fallback', evidenceAvailable=true; break (no model call) | pre-empts loop on envelope overflow / hard local-evidence closeout | in_engine | no | none | tool_evidence_fallback identical inline/engine | Engine handles the local-evidence fallback path; remaining work is extracting closeout policy. |
...
| shouldRepairPendingApprovalWaitTimeoutCheck | :833-853 (engine onRepairRound :3857-3876) | approval-wait-timeout closeout requested, latest permission is permission_query (no result after), candidate has no tool calls | force permission_result (re-check), nextToolChoice=permission_result, continue | S9 approval-wait-timeout family, after missing-approval-gate before premature/stale | in_engine | yes | PermissionEvidenceFact.lastQueryPending; taskPromptRequestsApprovalWaitTimeoutCloseout regex :8940-8950 | repairs pending answer from applied approval | Ported in Stage 8B slice 1; remaining work is typed permission facts, not parity. |
| shouldRepairPrematurePendingApprovalFinal | :855-878 (engine onRepairRound :3879-3898) | task requires carrying approval through, result mentions pending, permission_query exists but still pending, no session evidence, no marker | force permission_result re-check, nextToolChoice=permission_result, continue | S9 approval-wait-timeout family, before stale (:883) | in_engine | yes | PermissionEvidenceFact.lastResultStatus; hasPrematurePendingApprovalRepairPrompt fixed string 'approval-gated browser action is still pending' | repairs stale pending answers after daemon-supervised approval | Ported in Stage 8B slice 1; remaining work is typed approval status. |
| shouldRepairStalePendingApproval | :880-903 (engine onRepairRound :3901-3920) | result mentions pending, approval IS applied, approval-gated/continuation task, no marker | force sessions_spawn (execute approved action), nextToolChoice=sessions_spawn, continue | S9 approval-wait-timeout family, after premature | in_engine | yes | PermissionEvidenceFact.isApplied; mentionsPendingApproval regex :8928-8931; taskPromptIsAppliedApprovalBrowserContinuation :8786-8792 | repairs stale pending answers after approval | Re-arms the real approved action round; remaining work is typed approval/application facts. |
| shouldRepairApprovalWaitTimeoutCloseout | :930-953 (engine onRepairRound :3944-3962) | approval-wait-timeout closeout requested, wait-timeout evidence exists, closeout incomplete (!looksLikeCompleteApprovalWaitTimeoutCloseout), no marker | force tool-free synthesis (nextToolChoice='none'), append closeout prompt, continue | S9 approval-wait-timeout family, before forced-local-closeout (:955) | in_engine | yes | PermissionEvidenceFact.waitTimeoutStatus; hasApprovalWaitTimeoutEvidence scans toolTrace; looksLikeCompleteApprovalWaitTimeoutCloseout regex :7713 | — | Ported; remaining work is typed wait-timeout evidence. |
| shouldForceApprovalWaitTimeoutLocalCloseoutAfterFailedRepair | :955-983 (engine onRepairRound :3971-3980 + onTerminate tool_evidence_fallback) | approval-wait-timeout closeout requested, evidence exists, repair prompt sent but still incomplete | break loop, toolLoopCloseout={reason:'tool_evidence_fallback'}, buildApprovalWaitTimeoutLocalEvidenceCloseout | hard fallback after approval-wait-timeout closeout repair fails | in_engine | no | PermissionEvidenceFact.waitTimeoutStatus; collectApprovalWaitTimeoutRuntimeEvidence :7735-7756 | — | Ported via `{ closeout: "tool_evidence_fallback" }`; remaining work is extracting closeout policy. |
...
| shouldRepairMissingBrowserEvidenceDimensions | :720-745 (pseudo-tool-call closeout), :1080-1107 (wall-clock/round-limit closeout), engine completed :3923-3938 | completedEvidenceText non-empty AND findMissingBrowserEvidenceDimensions non-empty (frame/shadow/popup/dashboard), no marker | tool-free synthesis (nextToolChoice='none'), append prompt, continue | closeout-branch-only; intentionally outside the natural-finish cascade | in_engine | yes | BrowserEvidenceFact iframe/shadow/popup/dashboard; findMissingBrowserEvidenceDimensions :8464-8516 (4 dims, each requested/evidence/result/negated regex) | missing-browser-dimensions parity | completed-closeout-phase repair; multi-dim regex artifact |
...
| maybeAppendBrowserRecoveryVisibility | :6884-6916 (engine completed :4850, epilogue :1934/5161 context-adjacent) | browserRecoverySummaries>0 AND recovery/timeout keywords AND !isBrowserRecoveryVisible AND !expectsExactFinalAnswerShape | append 'Browser continuity: ... (resume mode: warm/cold)' | completed branch before failure-bucket & redaction | in_engine | yes | BrowserEvidenceFact.recoveryMetadata; isBrowserRecoveryVisible :6918-6932 regex; CapabilityFacts.browserTaskHints (taskPrompt regex :6893) | — | Ported; remaining work is structured recovery metadata. |
| maybeAppendBrowserFailureBucketVisibility | :6970-6993 (engine completed :4855, epilogue :5173) | collectBrowserFailureBucketNames finds bucket (target_not_found/attach_failed/...) AND !expectsExactFinalAnswerShape AND buckets not visible | append limitation/scope line via buildBrowserFailureBucketVisibilityLine | after recovery, before redaction | in_engine | yes | BrowserEvidenceFact.failureBucket (enum); collectBrowserFailureBucketNames :7077-7085 regex; browserFailureBucketVisible :7087-7109 | — | Ported; remaining work is typed browser failure buckets. |
| maybeAppendRequiredTimeoutFollowupVisibility | :7255-7300 (engine epilogue :5161) | taskRequestsTimeoutFollowupContinuation AND timeout evidence AND sessions_send AND missing guidance/scope/timeout phrases | append 'Continuation guidance: ...' + 'Unverified scope: ...' + 'Timeout recovery: ...' (only missing) | natural-finish epilogue after recovered-timeout visibility | in_engine | yes | SessionEvidenceFact.status, TaskFacts.requiresTimeoutFollowup; hasTimeoutContinuationGuidance :7385-7400, mentionsUnverifiedScope :7354-7357, mentionsTimeout :7402-7404 | — | Ported in finalization epilogue; remaining work is typed timeout-followup facts. |
| maybeAppendBrowserRecoveryResidualRiskVisibility | :7302-7337 (engine epilogue :5167) | !requestsStatusVisibleTextEvidenceUrlLines AND taskRequiresBrowserEvidence AND !residual-risk in result AND context has residual-risk AND (timeout result OR browser-recovery regex) | append 'Residual risk: this browser review is source-bounded ...' | after required-timeout-followup, before failure-bucket | in_engine | yes | BrowserEvidenceFact.isBounded/.recoveryStatus, TaskFacts.requiresBrowserEvidence; taskRequiresBrowserEvidence ~8535, requestsStatusVisibleTextEvidenceUrlLines ~8232 | — | Ported; remaining work is typed bounded-browser evidence facts. |
...
| behaviorName | inlineLocation | trigger | action | precedence | engineStatus | regex? | typedFactsNeeded | relatedTests | notes |
|---|---|---|---|---|---|---|---|---|---|
| generateWithEnvelopeRetry | :4269-4402 | RequestEnvelopeOverflowError on gateway.generate() | retry 3 levels (compact→minimal→reference-only); record reduction; persist memoryFlush | per-round synchronously after model call | in_engine | no | none | preserves tool history when envelope retry reduces (:3973) | wraps inline rounds + engine synthesis; calls flushPreCompactionMemorySafely on first overflow. |
| flushPreCompactionMemorySafely | :4404-4438 | overflow on first gateway.generate() | preCompactionMemoryFlusher.flush('request_envelope_overflow'); suppress errors | once per overflow, before retries | in_engine | no | none | triggers pre-compaction flusher on overflow (:162-221) | optional (nil check). |
| reductionSnapshot accumulation/carry-forward | :248-252 + :375/443/636/712/1074/1319/1383/1420/1462/1502/1821; engine :2604-2607/:5124 | each generateWithEnvelopeRetry returning reduction | capture into run reductionSnapshot, persist across rounds | after each retry | in_engine | no | none | preserves tool history when retry reduces (:3973) | Engine carries per-round reduction into final metadata and records the reduction boundary. |
...
| pruneToolResultMessagesForGateway | :12389-12434 | from prepareToolHistoryForGateway | prune >64KB OR (not-recent-2 AND >16KB) → summary JSON | first pass before compaction | in_engine | no | none | prunes older oversized (:4050+) | calls pruneToolResultsToTotalBudget. |
...
| compactOlderToolHistoryForGateway | :12436-12472 | messages.length >16 | compact older rounds → single summary, keep recent N | third stage after pruning | in_engine | no | none | implicit in prepareToolHistoryForGateway | greedy backward; 6KB cap. |
| summarizeToolResultPruning | :12321-12358 (engine :2568-2572/:5435/:5488/:5548) | after prepareToolHistoryForGateway (:355) | detect pruned + compaction → ToolResultPruningSnapshot | post-prep observability | in_engine | yes | structured pruning facts (now regex 'Earlier tool history compacted to fit') | preserves tool history when retry reduces (:3973) | Engine records pruning/compaction boundaries; remaining work is structured pruning facts. |
| withFinalToolRoundWarning | :12360-12387 (engine :2552-2556) | round===maxRounds-1 AND finite maxRounds | append user message warning final tool round | before prepareToolHistoryForGateway (:346) | in_engine | no | none | implicit end-of-round | Engine injects the final-round warning before preparing gateway history. |
...
| memoryFlushes accumulation | :254,:445-447 (engine :2608-2610 plus synthesis repairs) | generateWithEnvelopeRetry returns memoryFlush | push into memoryFlushes[] | after each retry | in_engine | no | none | triggers pre-compaction flusher (:162) | Engine accumulates pre-compaction flushes from tool rounds and synthesis repairs. |
| recordToolResultPruningBoundarySafely | :352-356 (engine :2568-2572/:5435/:5488/:5548) | after prepareToolHistoryForGateway in tool rounds | record ToolResultPruningSnapshot to runtimeProgressRecorder | post-prep observability | in_engine | yes | structured compaction facts (now text regex) | preserves tool history when retry reduces (:3973) | Engine records pruning boundaries; remaining work is structured compaction facts. |
| reduction metadata carry-forward in final response | :2306-2332 (engine :2604-2610,:5215-5216) | end of generate(); reductionSnapshot set | include requestEnvelopeReduction + preCompactionMemoryFlushes in metadata | terminal | in_engine | no | none | preserves tool history when retry reduces (:3973) | Engine carries round and synthesis reductions into final metadata. |
| recordReductionBoundarySafely | :4226-4227 / engine :5124-5125 | end of generate() if reductionSnapshot set | record reduction boundary to runtimeProgressRecorder | terminal | in_engine | no | none | preserves tool history when retry reduces (:3973) | Engine records this observability boundary; inline behavior is represented by metadata carry-forward. |
...
> historical record of what each row originally covered; it is NOT an open worklist.

| Retired fixture | Stage origin | Former blocker / covered behavior | Layer | Notes |
|---|---|---|---|---|
| **branch-2 timed-out-sibling-coverage** | Stage 7 branch-2 | `shouldContinueTimedOutSiblingSession` (~:1585 / engine :3004) sibling continuation | T5-continuation | Passes at 272/0; future work is replacing coverage-critical regex with typed TaskFacts (Group D). |
| **branch-3 supplemental-local-timeout-probe** | Stage 7 branch-3 | `enforceSupplementalLocalTimeoutProbeToolCall` (:473-484 / engine pipeline) + `shouldRunSupplementalLocalTimeoutProbe` (~:1605 / engine :3030) + prompt marker detection | T2-normalizer + T5-continuation | Passes at 272/0; future work is replacing probe/content-poor timeout text detection with typed facts (Group A). |
| **S9 post-execute missing-approval-gate** | Stage 7 S9 | `shouldRepairMissingApprovalGate` post-execute branch (~:1672 / engine :3112) + approval-wait-timeout family (:833-984 / engine :3857-3980) | T4-permission + T9-repair | Passes at 272/0; future work is `PermissionEvidenceFact` (Group B). |
| **S10 product-signal** | Stage 7 S10 | `missing_product_signal_browser_evidence_completed_rearm_s10` (~:1907 / engine :3715-3736) + `shouldRepairMissingProductSignalBrowserEvidence` (:776-802 / engine :3265-3287) | T9-repair + S10 re-arm | Passes at 272/0; future work is structured `productSignals{rendered,metrics,url,title}` (Group C). |

No deferred appender residual remains for parity: `maybeAppendBrowserRecoveryVisibility`, `maybeAppendBrowserFailureBucketVisibility`, `maybeAppendRequiredTimeoutFollowupVisibility`, and `maybeAppendBrowserRecoveryResidualRiskVisibility` all run on the engine. Their remaining work is typed browser/timeout facts, tracked in §4.
...
- **tool_evidence_fallback** — inline :420-431; engine `onModelCallError` :4915-4965 plus approval-wait hard-closeout :3971-3980. *Both local-evidence entry points now run on engine: envelope overflow routes through `buildLocalEvidenceCloseoutResult`, and `shouldForceApprovalWaitTimeoutLocalCloseoutAfterFailedRepair` returns `{closeout: "tool_evidence_fallback"}` for deterministic local closeout.*
...
4. **approval-wait-timeout family** — inline :833-984; engine :3857-3980.
   - **shouldRepairPendingApprovalWaitTimeoutCheck** — force permission_result.
   - **shouldRepairPrematurePendingApprovalFinal** — force permission_result.
   - **shouldRepairStalePendingApproval** — force sessions_spawn.
   - **shouldRepairApprovalWaitTimeoutCloseout** — tool-free synthesis.
   - **shouldForceApprovalWaitTimeoutLocalCloseoutAfterFailedRepair** — hard closeout to tool_evidence_fallback.
...
7. **shouldRepairSourceEvidenceCarryForward** — truthy-gated on sourceBoundedEvidenceText. inline :1202-1229; engine :3387-3412. *(round-dependent evidence formula aligned.)*
8. **shouldRepairWeakEvidenceSynthesis** — LAST tool-free repair. inline :1230-1252; engine :3413-3430.

*(Closeout-branch-only, intentionally outside natural-finish: `shouldRepairMissingBrowserEvidenceDimensions` — inline :720-745 (pseudo-tool-call closeout) + :1080-1107 (wall-clock/round-limit closeout); engine completed-closeout :3923-3938.)*
