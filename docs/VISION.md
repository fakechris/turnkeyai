...
在复杂、长时间运行的 Agent 任务中，我们重点解决这些问题：
...
TurnkeyAI 是一个本地工作台，目前提供 macOS Desktop App、浏览器中的
Control Center，以及面向高级用户和开发者的 CLI。

这些入口共享同一套本地 Runtime 和 Mission 数据。用户以 Mission 为中心
组织 Agent、Context、Approval、Artifact 和 Recovery；Browser、Document、
Desktop、File 与 API 是 Agent 完成任务时按需使用的执行面。
