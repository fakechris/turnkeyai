...
最终产品形态会是一个本地工作台。当前已经有 daemon-served Mission
Control web workbench；native Electron/Tauri shell 仍是后续包装层。

最新产品方向见 `docs/design/mission-control-product-design.md`：用户入口不应以 browser bridge / tabs / diagnostics 为中心，而应以 Mission 为中心组织多 agent 协同、context source、approval、artifact 和 runtime recovery。Browser、document、desktop、file、API 都是 agent 完成任务时可调用的执行面。

在研发路径上，我们会坚持：

- 先 daemon / runtime
- 再 TUI / debugging surface
- 再 daemon-served Mission Control web workbench
- 最后再做 native desktop shell

原因很直接：如果 runtime 核心不稳，native GUI 只会放大问题。当前
Mission Control web workbench 的角色，是在不引入 native packaging 复杂度的
前提下，把 mission、approval、runtime、context、evidence 和 recovery 变成用户可见入口。
