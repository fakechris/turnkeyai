...
- Every slice is gated by the **245-test oracle** (`llm-response-generator.test.ts` =
  197 inline behavior tests + 48 cutover parity tests) — must stay green with **zero
...
npx tsx --test packages/role-runtime/src/llm-response-generator.test.ts   # all green (245)
