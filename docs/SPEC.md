...
- Pre-compaction memory flush：request envelope 降级前先把 durable preferences / constraints / decisions / open items 写入 memory store
...
- 验收：TeamMessageStore 中 tool call / progress / result 是结构化字段，不靠纯文本约定；`/message/cancel-tools` 能取消进行中的工具；Mission Detail timeline 能 replay `tool call → tool progress → tool result → final answer`；`memory_search` / `memory_get` 能从同一套 durable memory resolver 中取回 prior decisions / preferences / constraints / evidence；request envelope 溢出触发 prompt reduction 前，会先执行 pre-compaction memory flush 并写入 durable thread memory
