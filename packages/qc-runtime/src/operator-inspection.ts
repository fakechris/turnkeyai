...
export function buildOperatorTriageReport(input: {
  ...
  if (
    ...
    focusAreas.push({
      ...
      reason:
        `Startup recovery restarted queued=${input.summary.roleRunStartupRecovery?.restartedQueuedRuns ?? 0}, running=${input.summary.roleRunStartupRecovery?.restartedRunningRuns ?? 0}, resuming=${input.summary.roleRunStartupRecovery?.restartedResumingRuns ?? 0}, cold-restart=${input.summary.roleRunStartupRecovery?.coldRestartRuns ?? 0}, orphaned=${input.summary.roleRunStartupRecovery?.orphanedThreadRuns ?? 0}, failed-orphaned=${input.summary.roleRunStartupRecovery?.failedOrphanedRuns ?? 0}, cleared-handoffs=${input.summary.roleRunStartupRecovery?.clearedInvalidHandoffs ?? 0}, queued-idled=${input.summary.roleRunStartupRecovery?.queuedRunsIdled ?? 0} role runs.`,
