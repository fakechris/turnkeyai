  ...
  async runActivation(input: RoleActivationInput): Promise<RoleRuntimeResult> {
    ...
    try {
      ...
      return {
        status: "ok",
        message,
        ...(nativeToolMessages.length > 0 ? { messages: [...nativeToolMessages, message] } : {}),
...
function buildNativeToolMessages(
  input: RoleActivationInput,
...
function omitToolUseTrace(metadata: Record<string, unknown>): Record<string, unknown> {
  const { toolUse: _toolUse, ...rest } = metadata;
  return rest;
}

interface ParsedToolUseTrace {
  rounds: Array<{
    round: number;
...
function parseToolUseTrace(value: unknown): ParsedToolUseTrace | null {
...
function buildRoundToolProgress(
  round: ParsedToolUseTrace["rounds"][number],
  baseTimestamp: number
