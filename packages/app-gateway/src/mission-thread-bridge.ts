...
function expandMessage(input: ExpandMessageInput): ActivityEvent[] {
  ...
  const toolUse = extractNativeToolUseTrace(message) ?? extractToolUseTrace(message.metadata);
...
function extractNativeToolUseTrace(message: TeamMessage): ToolUseTrace | null {
  ...
  const results = (message.toolProgress ?? [])
