...
test("resolveDaemonRequestAccess classifies representative route matrix entries", () => {
  const cases = [
    ...
    { method: "POST", pathname: "/mission-context-sources", expected: "operator" },
    { method: "GET", pathname: "/onboarding/state", expected: "read" },
    { method: "PUT", pathname: "/onboarding/state", expected: "operator" },
    { method: "POST", pathname: "/missions/bootstrap-demo", expected: "operator" },
    // PR K3.5 — mission lifecycle writes.
    { method: "POST", pathname: "/missions", expected: "operator" },
...
test("authorizeDaemonRequest enforces layered access while keeping health public", () => {
  ...
  assert.equal(
    authorizeDaemonRequest(
      { method: "POST", headers: { authorization: "Bearer operator-token" } } as never,
      new URL("http://127.0.0.1/phase1-baseline/run"),
      config
    ).authorized,
    false
  );
