...
test("workflow routes cancel active worker sessions when the tool registry misses the call", async () => {
  const messages = new Map<string, TeamMessage>();
  let workerCancelInput: { workerRunKey: string; reason?: string } | null = null;
  messages.set("assistant-1", {
    id: "assistant-1",
    threadId: "thread-1",
    role: "assistant",
...
test("workflow routes do not synthesize a cancelled result when fallback worker cancellation fails", async () => {
  const messages = new Map<string, TeamMessage>();
  messages.set("assistant-1", {
    id: "assistant-1",
    threadId: "thread-1",
  ...
  const deps = createDeps({
    ...
    workerRuntime: {
      async cancel() {
        throw new Error("worker cancel failed");
      },
      async listSessions() {
        return [
          {
