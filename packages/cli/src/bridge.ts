...
function buildSkillMarkdown(baseUrl: string): string {
  return [
    ...
    "",
    "`hover`, `scroll`, `dialog`, `popup`, `probe`, `console`, `pdf`, `click_coord`, `screenshot_clip`, `find_tab`",
    "",
    "Not yet exposed via the facade: `drag`, `download`, `storage`, `cookie`, `permission`, `network.*`.",
    "These actions are still reachable via `POST /browser-sessions/:id/send` with the raw action body until structured-arg validators land for each.",
    "",
