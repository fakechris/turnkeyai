...
function buildDefaultCapabilities(domain: string): PackManifestSection[] {
  ...
  return [
    {
      id: `${normalizedDomain}-intake`,
      summary: `Turn an incoming ${normalizedDomain} request into an executable brief and constraints list.`,
    },
    {
      id: `${normalizedDomain}-production`,
      summary: `Produce a first-pass ${normalizedDomain} asset or plan with explicit acceptance criteria.`,
    },
    {
      id: `${normalizedDomain}-qa`,
      summary: `Review ${normalizedDomain} outputs for quality, packaging, and handoff readiness.`,
    },
    {
      id: `${normalizedDomain}-adaptation`,
      summary: `Adapt ${normalizedDomain} outputs into derived formats, sizes, or downstream deliverables.`,
    },
  ];
}
...
function buildDefaultWorkflows(domain: string): PackManifestSection[] {
  ...
  return [
    {
      id: "brief-intake",
      summary: `Normalize the ${normalizedDomain} brief, required outputs, references, and constraints before execution.`,
    },
    {
      id: "execution-plan",
      summary: `Break the ${normalizedDomain} request into production steps, tool choices, and review gates.`,
    },
    {
      id: "quality-gate",
      summary: `Check the ${normalizedDomain} output against acceptance criteria and list any remaining gaps.`,
    },
  ];
}
...
- The deliverables, constraints, and review bar can be written down before execution.
- Another agent or pack may need to continue from the same artifacts later.

## Do Not Use This Pack When

- The request is mostly owned by a different domain pack and only mentions \`${manifest.domain}\` as a side effect.
- The caller cannot provide enough inputs to define outputs, constraints, or acceptance criteria.
- The work should stay exploratory and should not be normalized into a repeatable contract yet.

## Operating Contract

1. Start from a normalized brief with explicit outputs, references, and delivery constraints.
2. Pick the smallest tool and workflow set that can complete the pack without hidden assumptions.
...
- Every request produces a concrete brief, an execution plan, and a final handoff note.
- Tool-specific choices are explicit enough to replay in another environment.
- Domain assumptions stay in this pack instead of leaking into unrelated packs.
`;
}
...
- primary objective
- target user, audience, or downstream consumer
...
- Keep domain-specific assumptions under this reference instead of duplicating them in every workflow.
- Add schemas, vocabularies, naming rules, and output contracts here as the pack matures.
`;
}
