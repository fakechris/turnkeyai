...
function printHelp(exitCode: number): never {
  const lines = [
    ...
    "  npx @turnkeyai/cli app      No-install launcher when turnkeyai is not on PATH",
    "  npx @turnkeyai/cli daemon status  No-install daemon status check",
    "  npm run install:local-cli   Link source checkout as turnkeyai command",
    "  npm run app:install-launcher  Source-tree double-click launcher installer",
