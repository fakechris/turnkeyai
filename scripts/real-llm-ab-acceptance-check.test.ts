...
test("real LLM A/B acceptance check parses the JSON path", () => {
  ...
  assert.deepEqual(parseRealLlmAbAcceptanceCheckArgs(["--json", "/tmp/ab-report.json", "--suite", "browser-reliability"]), {
    jsonPath: "/tmp/ab-report.json",
    requiredSuite: "browser-reliability",
  });
  assert.deepEqual(parseRealLlmAbAcceptanceCheckArgs(["--json", "/tmp/ab-report.json", "--suite", "full-natural"]), {
    jsonPath: "/tmp/ab-report.json",
...
test("real LLM A/B acceptance check exposes help", () => {
  ...
  assert.match(buildRealLlmAbAcceptanceCheckHelpText(), /browser-reliability/);
  assert.match(buildRealLlmAbAcceptanceCheckHelpText(), /full-natural/);
